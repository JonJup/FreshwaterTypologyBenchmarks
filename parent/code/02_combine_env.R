################################################################################*
# Description:        Combines environmental predictor tables (one parquet
...
soil_pH                 <- dir_ls("env_data/soil_ph_europe/processedData/w_catchments/")
floodpl                 <- dir_ls("env_data/flood_area/processedData/w_catchment/")
streamp                 <- dir_ls("env_data/hydrography90m/processedData/spi_w_catchment/")
geology                 <- dir_ls("env_data/IHME1500_v11/w_catchments/")
glacier                 <- dir_ls("env_data/randolph glacier inventory/w_catchments/")
bioclim                 <- dir_ls("env_data/bioclim_chelsa/processedData/w_catchments/")
erosion                 <- dir_ls("env_data/Gloreda/processedData/w_catchements/")
roughness               <- dir_ls("env_data/DTM_Europe/processedData/roughness_w_catchment/")
elevation               <- dir_ls("env_data/DTM_Europe/processedData/elevation_w_catchment/")
snowdepth               <- dir_ls("env_data/CERRA/processedData/snow_depth_w_catchments/")
discharge               <- dir_ls("env_data/discharge copernicus/processedData/w_catchments/")
groundwater             <- dir_ls("env_data/BasinATLAS_Data_v10/processedData/groundwater_area_w_catchment/")
upstream_area           <- dir_ls("env_data/BasinATLAS_Data_v10/processedData/upstream_area_w_catchment/")
saturated_water_content <- dir_ls("env_data/soilHydraulicPropertiesEurope/processedData/soiHydraulicaProperties_w_catchments/")
valley_bottom_flatness  <- dir_ls("env_data/valley_bottom_flattness/processedData/valleyBottomFlattness_w_catchments/")
...
# rmv drop tiles whose filename matches a given pattern
rmv <- function(x, y) {
...
for (i in seq_len(n_tiles)) {
        ...
        # read all  parquet files for this tile 
        ...
        # merge all tables on shared catchment ID 
