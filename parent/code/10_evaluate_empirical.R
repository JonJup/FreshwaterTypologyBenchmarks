...
# ADD kmeans to list 
i.D.list <- list()
i.D.list[[1]] <- list(distance = NA, types = NA, Y = NA)
i.D.list[[1]]$distance  <- i.D
i.D.list[[1]]$types     <- i.clusters
i.D.list[[1]]$Y         <- i.Y
names(i.D.list) <- c("kmeans")
...
i.out[[length(i.out) + 1]] <- data.table(scheme = i.ss$scheme_id, value = i.fuzzy.out, typology = "fuzzy C means", n_types = i.ntypes["kmeans"], types = i.types.list["kmeans"],metric = "fuzzy_mantel")
